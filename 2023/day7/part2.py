...
def replaceJokers (hand: str) -> str:
	if 'J' not in hand or hand == 'JJJJJ':
		return hand

	counts = Counter(hand)
	countsSorted = counts.most_common(2)
	bestCard = countsSorted[0][0] if countsSorted[0][0] != 'J' else countsSorted[1][0]
...
with open('input.txt', 'r') as fin:
	for line in fin.readlines():
		...
		handStrength = handStrength.replace('J', '0')

		# Hand type is the most important so we add it to the beginning of handStrength
		handStrength = getHandType(replaceJokers(hand)) + handStrength
